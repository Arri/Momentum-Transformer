...
class TunerValidationLoss(kt.tuners.RandomSearch):
    ...
    def run_trial(self, trial, *args, **kwargs):
        # Add batch size to trial parameters
        ...
        # Extract the desired metric (e.g., validation loss)
        val_loss = history.history["val_loss"][-1]  # Ensure "val_loss" is in the model's metrics
...
class TunerDiversifiedSharpe(kt.tuners.RandomSearch):
    ...
    def run_trial(self, trial, *args, **kwargs):
        ...
        # Update callbacks with checkpointing
        ...
        # Average metrics across executions
        averaged_metrics = {}
        for metric, execution_values in metrics.items():
            averaged_metrics[metric] = np.mean(execution_values)

        # Update the trial and return the primary objective metric
        self.oracle.update_trial(trial.trial_id, metrics=averaged_metrics)
        return {"val_loss": averaged_metrics["val_loss"]}  # Ensure "val_loss" is used in your model

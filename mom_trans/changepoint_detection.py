...
def run_module(
    ...
    for window_end in range(lookback_window_length + 1, len(time_series_data)):
        ...
        print("[run_module] -----> window_end = ", window_end)
        print("[run_module] -----> window_date = ", window_date)
        try:
            if use_kM_hyp_to_initialise_kC:
                cp_score, cp_loc, cp_loc_normalised, _, _ = changepoint_loc_and_score(
                    ts_data_window,
                )
                print("\t\t Done use_kM_hyp_to_initialise_kC = ", use_kM_hyp_to_initialise_kC)
            else:
                cp_score, cp_loc, cp_loc_normalised, _, _ = changepoint_loc_and_score(
                    ts_data_window,
                    k1_lengthscale=1.0,
                ...
                print("\t\t NOT use_kM_hyp_to_initialise_kC")

        except:
            # write as NA when fails and will deal with this later
            print("\t---- write as NA when fails and will deal with this later")

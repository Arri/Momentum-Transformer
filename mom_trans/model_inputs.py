...
class ModelFeatures:
    ...
    def __init__(
        ...
        self._column_definition = [
            ...
            ("ticker_x", DataTypes.CATEGORICAL, InputTypes.ID),
        ]
        ...
        if add_ticker_as_static:
            ...
            df["static_ticker"] = df["ticker_x"]
            if static_ticker_type_feature:
                df["static_ticker_type"] = df["ticker_x"].map(
        ...
        if split_tickers_individually:
            ...
            if lags:
                tickers = (
                    trainvalid.groupby("ticker_x")["ticker_x"].count()
                    * (1.0 - train_valid_ratio)
                ) >= total_time_steps
                tickers = tickers[tickers].index.tolist()
            else:
                tickers = list(trainvalid.ticker_x.unique())

            train, valid = [], []
            for ticker_x in tickers:
                calib_data = trainvalid[trainvalid.ticker_x == ticker_x]
                T = len(calib_data)
            ...
            test = test[test.ticker_x.isin(tickers)]
        else:
            trainvalid = df.loc[years < test_boundary]
            ...
            if lags:
                tickers = (
                    valid.groupby("ticker_x")["ticker_x"].count() > self.total_time_steps
                )
                tickers = tickers[tickers].index.tolist()
                train = train[train.ticker_x.isin(tickers)]
                ...
                # )
                # tickers = tickers[tickers].index.tolist()
                tickers = list(train.ticker_x.unique())
            valid = valid[valid.ticker_x.isin(tickers)]
            test = test[test.ticker_x.isin(tickers)]
        ...
        test_with_buffer = pd.concat(
            [
                pd.concat(
                    [
                        trainvalid[trainvalid.ticker_x == t].iloc[
                            -(self.total_time_steps - 1) :
                        ],  # TODO this
                        test[test.ticker_x == t],
                    ]
                ).sort_index()
                for t in tickers
            ]
        ...
        if lags:
            for t in tickers:
                test_ticker = test[test["ticker_x"] == t]
                diff = len(test_ticker) - self.total_time_steps
                if diff < 0:
                    test = pd.concat(
                        [trainvalid[trainvalid["ticker_x"] == t][diff:], test]
                    )

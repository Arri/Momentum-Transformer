...
## Updates
### Work Environment
This code is based on git repo https://github.com/kieranjwood/trading-momentum-transformer/tree/master
The original code was using older versions of the required libraries (see `requirements.txt`). I wanted to run this code in an Anaconda environment with more up-to-date librarie versions.

I am working in an Ubuntu environment in *wsl2* (windows 10).

I created an Anaconda environment using **Python 3.8** (`conda create -n trade python==3.8`) and installed inside that environment all required libraries using `conda install` instead of `pip`, so that conda takes care of all required dependencies. I put all the conda instllations into a shell script, which can be run e.g. by making it executable using `chmod +x conda_requirements.sh`.

Running the installation script installed **tensorflow v2.11.1** in this environment, which is not the very latest TensorFlow version at this time, but has the two GPUs in my PC enabled.

### Yahoo Finance
The second issue I ran into, was that it did not seem to be possible to download NASDAQ trading data using `quantl`. Also the new NASDAQ [data-link API](https://pypi.org/project/Nasdaq-Data-Link/) did not seem to do the job. 

...
import pandas as pd
from settings.default import STOCK_TICKERS  # Updated for yfinance
from settings.fixed_params import MODLE_PARAMS
from mom_trans.backtest import run_all_windows
import numpy as np
from functools import reduce

# Define the asset class of each ticker here - for this example we assume all are equities
TEST_MODE = False
ASSET_CLASS_MAPPING = dict(zip(STOCK_TICKERS, ["EQUITY"] * len(STOCK_TICKERS)))
TRAIN_VALID_RATIO = 0.90
TIME_FEATURES = False
FORCE_OUTPUT_SHARPE_LENGTH = None
EVALUATE_DIVERSIFIED_VAL_SHARPE = True
NAME = "experiment_yfinance_100stocks"  # Changed to yfinance
...
def main(
    ...
    for v in versions:
        ...
        # Update feature file paths for yfinance
        if changepoint_lbws:
            features_file_path = os.path.join(
                "data",
                f"yfinance_cpd_{np.max(changepoint_lbws)}lbw/{np.max(changepoint_lbws)}_features.csv",  # Updated filename
            )
        else:
            features_file_path = os.path.join(
                "data",
                "yfinance_cpd_nonelbw.csv",  # Updated filename
            )
